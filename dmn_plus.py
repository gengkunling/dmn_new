...
class DMN_PLUS(object):
    ...
    def load_data(self, debug=False):
        ...
        if self.config.train_mode:
            self.train, self.valid, self.word_embedding, self.max_q_len, self.max_input_len, self.max_sen_len, self.num_supporting_facts, self.vocab_size = contract_input.load_data(self.config, split_sentences=True)
        else:
            self.test, self.word_embedding, self.max_q_len, self.max_input_len, self.max_sen_len, self.num_supporting_facts, self.vocab_size = contract_input.load_data(self.config, split_sentences=True)
    ...
    def add_placeholders(self):
        ...
        self.answer_placeholder = tf.placeholder(tf.int32, shape=(self.config.batch_size, 2))
    ...
    def get_predictions(self, output):
        begin_output, end_output = tf.split(output, 2, axis=1)
        begin_index = tf.argmax(tf.nn.softmax(begin_output), axis=1)
        end_index = tf.argmax(tf.nn.softmax(end_output), axis=1)
        pred = tf.stack([begin_index, end_index], axis=1)
    ...
    def add_loss_op(self, output):
        ...
        begin_output, end_output = tf.split(output, 2, axis=1)
        begin_answer, end_answes = self.answer_placeholder[:,0], self.answer_placeholder[:,1]

        loss += self.config.beta * tf.reduce_sum(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=begin_output, labels=begin_answer))
        loss += self.config.beta * tf.reduce_sum(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=end_output, labels=end_answes))
        #loss += self.config.beta * tf.reduce_sum(
        #    tf.nn.sparse_softmax_cross_entropy_with_logits(logits=output[1], labels=end_index))
    ...
    def add_answer_module(self, rnn_output, q_vec):
        ...
        a_begin_idx = tf.layers.dense(tf.concat([rnn_output, q_vec], 1),
                self.config.answer_size,
                activation=None)

        a_end_idx = tf.layers.dense(tf.concat([rnn_output, q_vec], 1),
                                    self.config.answer_size,
                                      activation=None)

        ######################################
        # need fiex later ? debug
        output = tf.concat([tf.squeeze(a_begin_idx), tf.squeeze(a_end_idx)], 1)
        ######################################
    ...
    def run_epoch(self, session, data, num_epoch=0, train_writer=None, train_op=None, verbose=2, train=False):
        ...
        qp, ip, ql, il, im,  r = qp[p], ip[p], ql[p], il[p], im[p],  r[p]
        a = [a[i] for i in p]
        ...
        for step in range(total_steps):
            ...
            feed = {self.question_placeholder: qp[index],
                  ...
                  self.answer_placeholder: [a[i] for i in index],
